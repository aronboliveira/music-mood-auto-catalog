...
for i in range(1, 26):
    ...
    singles_files.append({"filename": filename, "artist": artist, "genre": genre, "path": filepath, "size": os.path.getsize(filepath)})
...
for i in range(1, 9):
    ...
    generate_mp3(filepath, duration)
    albums_files.append({"filename": filename, "artist": artist, "genre": genre, "path": filepath, "size": os.path.getsize(filepath)})
...
for root, dirs, files in os.walk(classified_dir):
    ...
    subindent = ' ' * 4 * (level + 1)
    for f in files:
        main_taxonomy.append(f"{subindent}{f}")

with open(os.path.join(maps_dir_20260324, 'main-taxonomy.txt'), 'w') as f:
    f.write("\n".join(main_taxonomy))

with open(os.path.join(maps_dir_20260324, 'sliced-taxonomy.txt'), 'w') as f:
    f.write("No slices in mock dataset.\n")

with open(os.path.join(maps_dir_20260324, 'track-mods.txt'), 'w') as f:
    f.write("Mock dataset generation complete. No track mods required.\n")
    
mock_track_moods = {}
for f in singles_files + albums_files:
    mock_track_moods[f['filename']] = {
        "artist": f['artist'],
        "genre": f['genre'],
        "moods": genres[f['genre']]
    }
with open(os.path.join(maps_dir_20260324, 'track-moods.json'), 'w') as f:
    json.dump(mock_track_moods, f, indent=2)

with open(os.path.join(maps_dir_20260324, 'track-moods.html'), 'w') as f:
    f.write("<html><body><h1>Mock Dataset Moods</h1><p>Replace with actual visualization if needed.</p></body></html>")
...
for root, _, files in os.walk(maps_base):
    for f in files:
        if f.endswith('.json') and f != 'track-moods.json':
            with open(os.path.join(root, f), 'w') as json_file:
                json.dump(mock_tree, json_file, indent=2)
        elif f.endswith('.yml') or f.endswith('.yaml'):
            if yaml:
                with open(os.path.join(root, f), 'w') as yml_file:
...
with open(readme_mock_path, 'w') as f:
    f.write("# Mock Dataset\n\nThis dataset is a mock generation for public demonstration. The real files have been removed to avoid copyrighted or inappropriate names/content. Fictional artists, genres, and audio files have been deterministically generated.\n")

tech_overview_path = os.path.join(BASE_DIR, 'docs', 'specifications', 'technical-overview.md')
if os.path.exists(tech_overview_path):
    with open(tech_overview_path, 'r') as f:
        content = f.read()
    if "MOCK DATASET" not in content:
        with open(tech_overview_path, 'w') as f:
            f.write("**NOTE: THIS PROJECT IS CURRENTLY USING A MOCK DATASET FOR PUBLIC DEMONSTRATION.**\n\n" + content)
